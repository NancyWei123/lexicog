...
fn normalize_tools_for_byteplus(tools: Vec<Value>) -> Vec<Value> {
    tools
        .into_iter()
        .map(|tool| {
        ...
        .filter_map(|(index, item)| {
            ...
            Some(ToolCall {
                call_id: item
                    .call_id
                    .or(item.id)
                    .unwrap_or_else(|| format!("byteplus_call_{}", index)),
                name,
                arguments,
            })
        })
...
impl BytePlusClient {
    pub async fn new(db: &SqliteInterface) -> Result<Self> {
        let (api_key, base_url) =
            impl_get_vendor_api(db, constant::Vendor::BytePlus.to_string().as_str())
                .await
                .context("initialize BytePlus client: load vendor credentials from database")?
                .ok_or(anyhow!("missing API credentials for vendor BytePlus"))
                .context("initialize BytePlus client: validate stored credentials")?;
        Ok(Self {
            api_key,
...
impl super::TextToTextClient for BytePlusClient {
    fn execute_streaming_text_to_text_task(
        ...
        let normalized_tools = tools.map(normalize_tools_for_byteplus);
        let has_tools = normalized_tools
            .as_ref()
        ...
        if let Some(tools_array) = normalized_tools {
        ...
        if has_tools {
            request_builder = request_builder.header("ark-beta-mcp", "true");
        }

        Box::pin(async move {
            let response = request_builder.send().await.with_context(|| {
                format!(
                    "BytePlus text-to-text streaming request failed (model: {})",
                    model_id_for_error
                )
            })?;
            ...
            if !status.is_success() {
                let error_text = response
                    ...
                    .with_context(|| {
                        format!(
                            "BytePlus text-to-text streaming request failed while reading error body (model: {})",
                            model_id_for_error
                        )
                    })?;
                ...
                .with_context(|| {
                    format!(
                        "BytePlus text-to-text streaming API returned non-success status (model: {})",
                        model_id_for_error
                    )
                });
            }
                ...
                match stream.next().await {
                    Some(chunk) => {
                        let chunk = chunk.with_context(|| {
                            format!(
                                "BytePlus text-to-text streaming response chunk read failed (model: {})",
                                model_id_for_error
                            )
                        })?;
                    ...
                    match event_type.as_str() {
                        "response.output_text.delta" => {
                            let delta_event = serde_json::from_str::<DeltaEvent>(&data)
                                .context("parse BytePlus SSE event: response.output_text.delta")?;
                            if let Some(delta) = delta_event.delta {
                                saw_text_delta = true;
                                result.text.push_str(&delta);
                                channel
                                    .send(Some(delta))
                                    .context("forward BytePlus text delta to frontend channel")?;
                            }
                        }
                        "response.completed" | "response.incomplete" => {
                            let completed_event =
                                serde_json::from_str::<ResponseCompletedEvent>(&data)
                                    .with_context(|| {
                                        format!("parse BytePlus SSE event: {}", event_type)
                                    })?;
                                ...
                                if !saw_text_delta {
                                    ...
                                    if !fallback_text.is_empty() {
                                        ...
                                        channel.send(Some(fallback_text)).context(
                                            "forward BytePlus text fallback to frontend channel",
                                        )?;
                                    }
                                }
                        ...
                        "response.failed" | "error" => {
                            return Err(anyhow!(extract_stream_error_message(&data))).context(
                                "BytePlus text-to-text streaming API emitted a failure event",
                            );
                        }
                        _ => {}
                    }
            ...
            channel
                .send(None)
                .context("send BytePlus text streaming completion signal to frontend channel")?;
            Ok(result)
        })
    }
